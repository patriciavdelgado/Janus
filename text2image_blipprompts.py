...
def generate(
    ...
    tokens = torch.zeros((parallel_size*2, len(input_ids)), dtype=torch.int).to('cpu')
    ...
    generated_tokens = torch.zeros((parallel_size, image_token_num_per_image), dtype=torch.int).to('cpu')
    ...
    os.makedirs('generated_samples_SUIM_folder2_blip_errors_2', exist_ok=True)
    for i in range(parallel_size):
        save_path = os.path.join('generated_samples_SUIM_folder2_blip_errors_2', f"prompt{prompt_index+1}_img_{i}.jpg")
...
# Read the CSV file
csv_file_path = 'captions_folder2_blip_errors_2.csv'
